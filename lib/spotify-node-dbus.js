...
let getDBusInterfacePromise = null;
...
let execScript = function (scriptName, params, callback) {
    ...
    if (process.platform === "darwin") {
        ...
        (async function () {
            try {
                if (dbusService == null) {
                    dbusService = dbus.sessionBus().getService(
                        "org.mpris.MediaPlayer2.spotify"
                    );
                }

                if (getDBusInterfacePromise == null) {
                    getDBusInterfacePromise = promisify(dbusService.getInterface).bind(dbusService)(
                        "/org/mpris/MediaPlayer2",
                        "org.mpris.MediaPlayer2.Player"
                    );
                }

                let spotifyDBusInterface;
                try {
                    spotifyDBusInterface = await getDBusInterfacePromise;
                } catch (e) {
                    getDBusInterfacePromise = null;
                    if (scriptName === "isRunning") {
                        callback(null, false);
                    } else callback(new Error("Connection failed"), null)
                    return;
                }

                if (scriptName === "isRunning") {
                    await readDBusProp(spotifyDBusInterface, "PlaybackStatus");
